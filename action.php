...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM categorie LIMIT 10";
	$run_query = mysqli_query($conn,$category_query) or die(mysqli_error($coNn));
	echo "
		<div class='nav nav-pills nav-stacked' style='background-color: #9B9B9B'>
			<li class='active' ><a href='#' style='background-color: #606060 '><h4 >Product Categories </h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='category' style='color:#FEFEFF' cid='$cid'>$cat_name</a></li>
			";
		}
		echo "</div>";
	}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info' >
								<div class='panel-heading' style=' background-color: #606060;height:60px '>$pro_title</div>
								<div class='panel-body'>
									<img src=$pro_image style='width:220px; height:250px;'/>
								</div>
								<div class='panel-heading' style=' background-color: #606060 '>".CURRENCY.". $pro_price.00/-
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;background-color: #606060 " name="login_user_with_product" class="btn btn-info btn-lg" value="Ready to Checkout" >
							</form>';
					...
					echo '
						...
						<form action="payer.php" method="post">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/TPECOMMERCE%20-%20Copie/payer.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/TPECOMMERCE%20-%20Copie/payer.php">
